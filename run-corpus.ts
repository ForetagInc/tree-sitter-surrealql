...
function lezerCanonical(input: string): CNode | null {
	...
	function walk(): CNode | null {
		...
		if (node.children.length === 0 && STRIP_WHEN_EMPTY.has(normName)) return null;
...
function tsCanonical(input: string): CNode | null {
	...
	let s = raw.replace(/\[[^\]]*\]/g, "").replace(/[a-zA-Z_][a-zA-Z0-9_]*:/g, "");
	let i = 0;
	function skipWs() {
...
const dirs = target
	? [target]
	: fs.readdirSync(LEZER_CORPUS).filter((d) =>
